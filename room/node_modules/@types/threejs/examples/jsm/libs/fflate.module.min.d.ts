...
export interface DeflateOptions {
     ...
     * In both cases, higher values usually take disproportionately longer than the reduction in final size that
     * results.
     *
    ...
    /**
     * The memory level to use, ranging from 0-12. Increasing this increases speed and compression ratio at the cost of
     * memory.
     *
     * Note that this is exponential: while level 0 uses 4 kB, level 4 uses 64 kB, level 8 uses 1 MB, and level 12 uses
     * 16 MB. It is recommended not to lower the value below 4, since that tends to hurt performance. In addition,
     * values above 8 tend to help very little on most data and can even hurt performance.
     *
 ...
 * @param data The data to decompress
 * @param out Where to write the data. Saves memory if you know the decompressed size and provide an output buffer of
 *     that length.
